...
#include "src/Utils/Algorithms/FindPair.h"
...
private:
	std::map<std::string, std::function<LayerUPtr(FuncParamType)>> creators {
		{ StepNN::ConvLayerSettings::SETTINGS_ID, static_cast<LayerUPtr(*)(FuncParamType)>(CreateConvLayerTorch) },
	};
};

LayerCreator<std::string> g_idCreator;
LayerCreator<StepNN::BaseLayerSettings, const StepNN::BaseLayerSettings&> g_settingsCreator;
